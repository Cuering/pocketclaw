...
class _WorkflowsScreenState extends State<WorkflowsScreen> {
  ...
  Future<void> _createWorkflow({StateSetter? dialogSetState}) async {
    ...
    if (workflow != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '✅ 已创建工作流：${workflow.name}（${workflow.stepSkillIds.length} 步）',
          ),
        ),
      );
    } else {
  ...
  Future<void> _runWorkflow(WorkflowModel workflow) async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(SnackBar(content: Text(task.result ?? '工作流已完成')));
  }
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 4),
                                      Text(
                                        '${workflow.stepSkillIds.length} 个技能'
